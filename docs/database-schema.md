...
## 全店舗データのベクトルDB化（004_places_search_results_embedding.sql）

| 追加内容 | 用途 |
|---|---|
| `places_search_results.embedding`（`VECTOR(768)`） | 収集済み全店舗データのベクトルDB化。店舗名・業種タグ・住所の連結テキストから`google_ml.embedding(...)`でDB側生成する。既存行のバックフィルは閉鎖店舗探索ジョブが実行の度にバッチで行う（起動時マイグレーションで同期実行すると起動タイムアウトを超過するため） |
| HNSWインデックス（コサイン距離） | 類似店舗検索・統計での`<=>`演算子によるソートの高速化 |

管理画面の統計情報（ベクトルDB分布・クラスタ数）は、このカラムを対象に全店舗データを集計します。
