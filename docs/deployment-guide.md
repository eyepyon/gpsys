...
└── modules/
    ...
    ├── cloudrun_frontend/                     # 公開フロント画面(inuki)
...
└── frontend/Dockerfile        # 公開フロント用nginxコンテナイメージ
...
| `frontend_service_name` | - | `inuki` | 公開フロントのCloud Runサービス名 |
| `frontend_image` | ○ | - | 公開フロントのコンテナイメージURL |
...
terraform apply -target=module.artifact_registry -var="project_id=<your-project-id>" \
  ...
  -var="vacant_sync_image=placeholder" -var="frontend_image=placeholder" \
...
docker build -f docker/frontend/Dockerfile -t us-central1-docker.pkg.dev/<your-project-id>/regional-revitalization/frontend:latest .
docker push us-central1-docker.pkg.dev/<your-project-id>/regional-revitalization/frontend:latest
```
...
terraform plan \
  ...
  -var="frontend_image=us-central1-docker.pkg.dev/<your-project-id>/regional-revitalization/frontend:latest" \
  -var="storage_bucket_name=<bucket-name>" \
  -out=tfplan
...
| `GCP_DEPLOYER_SERVICE_ACCOUNT` | デプロイ用サービスアカウントのメールアドレス（Terraformの`github_actions_wif`モジュールで作成した場合は手順5の`github_actions_deployer_service_account_email`出力値、手動構築したWIF環境を使う場合はそのサービスアカウントのメールアドレス） | `google-github-actions/auth`アクションのなりすまし対象サービスアカウント |
| `GCP_BUCKET_NAME` | 手順1で作成したtfstate/地域資源ファイル共用バケット名 | `terraform init`のバックエンド設定、および`terraform apply`の`storage_bucket_name`変数 |
| `ENABLE_GITHUB_ACTIONS_WIF` | `true`または`false`（未設定時は`false`扱い） | `terraform apply`の`enable_github_actions_wif`変数。Terraformの`github_actions_wif`モジュールでWIF環境を管理する場合のみ`true`にする。手動構築したWIF環境を使う場合は`false`のままにし、Terraformに重複したWIFリソースを作成させない |
...
2. **コンテナイメージのビルド・プッシュ**: APIRun・InferRun・居抜き物件同期サービス・フロントの4イメージを、コミットSHAをタグとしてビルドし、Artifact Registryへプッシュ（`latest`タグも同時更新）
3. **Terraform適用**: 新しいイメージタグを`app_image`/`inference_image`/`vacant_sync_image`/`frontend_image`変数に渡し、`terraform plan`→`terraform apply`を実行
...
| モジュール | 内容 |
|---|---|
| `modules/network` | Direct VPC egress、Cloud SQLのプライベートIP接続に必要なPrivate Services Access（VPCピアリング） |
...
| `Error acquiring the state lock` | 別のTerraform実行がapply中、または異常終了した実行のロックが残っている | 実行中のDeployがないことを確認して待機・再実行する。古いロックの場合のみ表示されたLock IDを`terraform force-unlock <LOCK_ID>`で解除する。`-lock=false`は使用しない |
